...
from modules.deep_learning.util_identificacao_sentimento import *
...
dirbase = os.path.join(os.path.abspath('.'), 'dados\\sentimentos')
data_dir = os.path.join(dirbase, 'dataset')
log_dir = os.path.join(dirbase, 'modelo')
...
print('Diretorio arquivos: {}'.format(dirbase))
tf.flags.DEFINE_string('data_dir', 'dataset/', data_dir)
tf.flags.DEFINE_string('logs_dir', 'modelo/', log_dir)
tf.flags.DEFINE_string('mode', 'train', 'mode: train (Default)/ test')

# FLAGS.__flags['data_dir'].flag_values_dict()
...
#%% Função auxiliares para construção do modelo
...
weights = {
    ...
    'wc2': weight_variable([5,5,1,32], name='W_conv2'),
...
def main(argv=None):
    ...
    # Carrega os dados
    train_images, train_labels, valid_images, valid_labels, test_images = read_data(data_dir, True)
    ...
    with tf.Session() as sess:
        ...
        plt.title('Erro na Valsidação Por Iteração')
        plt.xlabel('Iteração')
